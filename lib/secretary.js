...
const loadActiveUser = async (userId, connection = getPool()) => {
  const [rows] = await connection.query(
    `SELECT id, username, name, department, role, active, locked_at, created_at, updated_at
...
export async function loginSecretary({ username, password } = {}) {
  ...
  const [rows] = await pool.query(
    `SELECT id, username, password, name, department, role, active, failed_login_attempts,
  ...
  try {
    const [result] = await pool.query(
      `INSERT INTO secretary_users (username, password, name, department, role, active)
       VALUES (?, ?, ?, ?, ?, ?)`,
      [username, passwordHash, name, department, role, active],
    )
    const [rows] = await pool.query(
      `SELECT id, username, name, department, role, active, locked_at, created_at, updated_at
...
export async function updateSecretaryUser(req, userId) {
  ...
  const sets = ['username = ?', 'name = ?', 'department = ?', 'role = ?', 'active = ?', 'failed_login_attempts = 0', 'locked_at = NULL']
  const values = [username, name, department, role, active]
  const password = String(input.password || '')
  if (password) {
    assertPasswordPolicy(password)
    sets.push('password = ?')
    ...
    for (let index = 0; index < rows.length; index += 1) {
      ...
      try {
        ...
        if (existing) {
          ...
          const sets = ['name = ?', 'department = ?', 'role = ?', 'active = ?', 'failed_login_attempts = 0', 'locked_at = NULL']
          const values = [name, department, role, active]
          if (password) {
            assertPasswordPolicy(password)
            sets.push('password = ?')
            values.push(await hashPassword(password))
          ...
          await connection.query(
            `INSERT INTO secretary_users (username, password, name, department, role, active)
             VALUES (?, ?, ?, ?, ?, ?)`,
            [username, await hashPassword(password), name, department, role, active],
          )
          inserted += 1
        }
      } catch (error) {
