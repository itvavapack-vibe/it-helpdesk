...
import { assertPasswordPolicy, hashPassword, isPasswordHash, normalizeAdminRole } from './auth.js'
...
async function prepareDataForWrite(table, data) {
  ...
  if (prepared.password && !isPasswordHash(prepared.password)) {
    assertPasswordPolicy(prepared.password)
    prepared.password = await hashPassword(prepared.password)
    prepared.password_changed_at = new Date()
  }
  return prepared
}
...
export async function updateTable(table, data, query) {
  ...
  const setClause = keys
    .map((key) => {
      ...
      if (table === 'issues' && key === 'assigned_admin') {
        return `\`${key}\` = CASE WHEN \`${key}\` IS NULL OR TRIM(\`${key}\`) = '' THEN ? ELSE \`${key}\` END`
      }
      return `\`${key}\` = ?`
    })
    .join(', ')
