...
const VALID_ROLES = new Set(['superadmin', 'it_support', 'it_supervisor', 'it_manager', 'it_software', 'hr'])
const ROLE_ALIASES = {
...
export async function getAdminProfile(adminId) {
  ...
  const pool = getPool()
  const [rows] = await pool.query(
    'SELECT id, username, name, role, created_at FROM `admins` WHERE `id` = ? LIMIT 1',
    [adminId],
  )
