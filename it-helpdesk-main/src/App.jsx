...
import { sendTelegramMessage, buildNewIssueMessage, buildStatusUpdateMessage } from './telegramNotify';
...
function App() {
    ...
    const addIssue = async (newIssue) => {
        ...
        // ส่งแจ้งเตือน Telegram เมื่อมีการแจ้งซ่อมใหม่
        sendTelegramMessage(buildNewIssueMessage(issueWithId));
    ...
    const updateIssueStatus = async (id, newStatus, adminName = null) => {
        ...
        // ส่งแจ้งเตือน Telegram เมื่ออัปเดตสถานะ
        const targetIssue = issues.find(issue => issue.id === id);
        if (targetIssue) {
            sendTelegramMessage(buildStatusUpdateMessage(targetIssue, newStatus, adminName));
        }
